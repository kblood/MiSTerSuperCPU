#!/usr/bin/env python3
"""IDENTICAL to test_long_store_only but writes $A5 instead of $3E.
If this PASSES, $3E in test_long_store_only was a real write (and the previous
"REU residue" hypothesis is wrong).
If this FAILS but $3E PASSES, the bug is value-dependent — really weird.
"""
import os
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_long_store_a5.prg')
...
emit(0xA9, 0xA5)                     # <<-- value $A5 (was $3E)
emit(0xA0, 0x00)
emit(0x97, 0xFB)
emit(0x18, 0xFB)
emit(0xC2, 0x20)

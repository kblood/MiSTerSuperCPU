#!/usr/bin/env python3
"""Same as test_abs_long_superram but with the $D07A 1MHz toggle before $D07B.

Hypothesis: The missing $D07A write is what makes test_post_reu_store / test_abs_long_superram
fail while test_long_store_only passes. Either there's a real state-machine effect, or
the extra $D07x cycles are providing essential settling time.
"""
import os
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_abs_long_with_1mhz.prg')

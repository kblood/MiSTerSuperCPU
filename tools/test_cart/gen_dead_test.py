#!/usr/bin/env python3
"""
Simple diagnostic cartridge that doesn't rely on character ROM.

Uses bitmap mode to display a pattern directly from memory.
This tests basic cartridge execution without character ROM dependency.

Usage:
  python gen_dead_test.py
"""
...
def assemble_dead_test() -> Asm6502:
    """
    Dead Test — No character ROM required.

    Sets up bitmap mode and fills it with a test pattern.
    Demonstrates cartridge execution without relying on character ROM.
...
def main():
    asm  = assemble_dead_test()
    stem = "scpu_dead_test"
    desc = "Dead Test (bitmap mode, no character ROM)"

    rom = make_rom(asm)
    crt = make_crt(rom, "SCPU DEAD TEST")

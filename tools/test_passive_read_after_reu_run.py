"""Load doom.reu, then run passive_read.prg. If doom.reu data persists,
$2A:$6C00 should reflect whatever doom.reu put there. The test_passive_read
PRG only does LDA long $2A:$6C00; the latched mem_44_r captures the read."""
import os, time, paramiko, base64, re
from collections import Counter
HOST,USER,PASS = '192.168.50.130','root','1'
RBF='/media/fat/_Test/C64.rbf'; CFG='/media/fat/config/C64.cfg'
PRG_LOCAL=os.path.join(os.path.dirname(os.path.abspath(__file__)),'test_passive_read.prg')
PRG_REMOTE='/tmp/test_passive_read.prg'
REU_REMOTE='/media/fat/games/C64/doom.reu'
REU_MGL='/tmp/load_doom_reu_only.mgl'
PRG_MGL='/tmp/load_passive_after_reu.mgl'
OUT=os.path.join(os.path.dirname(os.path.abspath(__file__)),'test_passive_read_after_reu_run')
def run(c,cmd,t=30):
...
print('load passive_read PRG ...'); run(c,'echo load_core '+PRG_MGL+' > /dev/MiSTer_cmd'); time.sleep(15)
...
if gv:
    ...
    cv=Counter(g[1] for g in gv); print('hist 2A:',cv.most_common(5))

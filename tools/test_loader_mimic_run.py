...
PRG_LOCAL = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_loader_mimic.prg')
PRG_REMOTE = '/tmp/test_loader_mimic.prg'
REU_MGL = '/tmp/load_doom_reu_only.mgl'
PRG_MGL = '/tmp/load_test_loader_mimic.mgl'
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_loader_mimic_run')
...
def write_mgl(c, path, body):
    ...
    for line in body.split('\n'):
        if line.strip(): run(c, "echo '"+line+"' >> "+path)
def main():
    ...
    if gv:
        ...
        c44 = Counter(g[1] for g in gv)
        print('$2A:$6C00 hist:',', '.join('$'+v.upper()+'='+str(n) for v,n in c44.most_common(5)))

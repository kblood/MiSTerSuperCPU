#!/usr/bin/env python3
"""Run test_reu_fetch on hardware. Pre-loads doom.reu via MGL so REU SDRAM
has bank $2A populated, then runs PRG that FETCHes 16 bytes from REU
bank $2A:$6C00 to c64 $6C00 and verifies. Border GREEN = pass."""
...
PRG_LOCAL = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         'test_reu_fetch.prg')
PRG_REMOTE = '/tmp/test_reu_fetch.prg'
MGL_REMOTE = '/tmp/test_reu_fetch.mgl'
...
def main():
    ...
    if rp:
        sftp = c.open_sftp()
        lp = os.path.join(OUT, 'reu_fetch.png')
        sftp.get(rp, lp)
        sftp.close()
        print('saved -> ' + lp)
    c.close()

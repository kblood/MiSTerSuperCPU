#!/usr/bin/env python3
"""Cleanest test: REU loaded, then NO toggles, just long-store + read.
Same as test_long_store_only but expected to be runnable post-REU-load.
"""
import os
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_post_reu_store.prg')
...
emit(0x8D, 0x7E, 0xD0)
emit(0x8D, 0x7B, 0xD0)               # turbo only (no 1MHz)
emit(0xA9, 0x03, 0x8D, 0x20, 0xD0)
# Single long-store: A=$66 (use distinct value, NOT $3E that REU may have)
emit(0xA9, 0x00, 0x85, 0xFB)
emit(0xA9, 0x6C, 0x85, 0xFC)
emit(0xA9, 0x2A, 0x85, 0xFD)
emit(0xA9, 0x66)                     # <<-- use $66, not $3E
emit(0xA0, 0x00)
emit(0x97, 0xFB)
...
emit(0xE2, 0x20)
emit(0xC9, 0x66)                     # check $66 (would have been $3E from REU only)

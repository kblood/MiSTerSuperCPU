#!/usr/bin/env python3
"""Doom full-load verification — 240s settle for REU->SuperRAM + bank20 boot.

Also captures intermediate snapshots at 60s, 120s, 180s, 240s so we can see
the loader's progress and the hand-off to native game code.
"""
...
def main():
    ...
    try:
        for mode, val in [('t65', 0x08), ('scpu', 0x0C)]:
            print('--- doom_full [{}] ---'.format(mode))
            setcfg(c, val)
            run(c, 'echo load_core ' + RBF + ' > /dev/MiSTer_cmd')
            time.sleep(8)
            run(c, 'echo load_core /media/fat/_Test/doom_full_abs.mgl > /dev/MiSTer_cmd')
            t0 = time.time()
            for stage_seconds in [60, 120, 180, 240]:
                wait = stage_seconds - (time.time() - t0)
                if wait > 0:
                    print('  settling to t={}s (wait {:.1f}s)'.format(stage_seconds, wait))
                    time.sleep(wait)
                img = shot(c, 'doom_full_{}_{}s'.format(mode, stage_seconds))
                print('   t={}s -> {}'.format(stage_seconds, os.path.basename(img) if img else 'NO-SHOT'))
    finally:
        c.close()

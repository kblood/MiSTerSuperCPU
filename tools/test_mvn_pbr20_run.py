#!/usr/bin/env python3
"""Run test_mvn_pbr20 PRG on hardware. Border = pass/fail indicator."""
...
PRG_LOCAL = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         'test_mvn_pbr20.prg')
PRG_REMOTE = '/tmp/test_mvn_pbr20.prg'
MGL_REMOTE = '/tmp/test_mvn_pbr20.mgl'
...
def main():
    ...
    if rp:
        sftp = c.open_sftp()
        lp = os.path.join(OUT, 'pbr20_mvn.png')
        sftp.get(rp, lp)
        sftp.close()
        print('saved -> ' + lp)
    c.close()

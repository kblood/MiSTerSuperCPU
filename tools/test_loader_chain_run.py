#!/usr/bin/env python3
"""Run test_loader_chain.prg on v277 hardware. Loads doom.reu first via
a SEPARATE MGL, waits for SDRAM to be populated, then runs the chain
test PRG via a single-file MGL (autorun via load_prg path).

Reads the v277 mem_44 latch (bank $2A:$6C00) in UART output to
determine what value is at bank $2A SuperRAM after the long-store.

PASS: green border + UART G byte 2 = $3E. Long-store path works.
FAIL: red border + UART byte tells us what readback returned.
"""
...
PRG_LOCAL = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         'test_loader_chain.prg')
PRG_REMOTE = '/tmp/test_loader_chain.prg'
REU_MGL = '/tmp/load_doom_reu_only.mgl'
PRG_MGL = '/tmp/load_test_loader_chain.mgl'
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_loader_chain_run')
...
def write_mgl(c, path, body):
    run(c, 'rm -f ' + path)
    for line in body.split('\n'):
        if line.strip():
            run(c, "echo '" + line + "' >> " + path)
...
def main():
    ...
    write_mgl(c, REU_MGL, reu_mgl)
    write_mgl(c, PRG_MGL, prg_mgl)
    ...
    time.sleep(8)
    print('load doom.reu (32MB takes a while) ...')
    run(c, 'echo load_core ' + REU_MGL + ' > /dev/MiSTer_cmd')
    time.sleep(40)  # REU files are large
    print('load test PRG (autorun) ...')
    run(c, 'echo load_core ' + PRG_MGL + ' > /dev/MiSTer_cmd')
    time.sleep(15)
    ...
    if g_vals:
        ...
        print('  $2A:$6C00 = ${}  <<<<< KEY VALUE'.format(last[1]))
        print('  $00:$6C03 = ${}'.format(last[2]))
        if b_vals:
            print('  $2A:$6C03 = ${}'.format(b_vals[-1]))
        c44 = Counter(g[1] for g in g_vals)
        print('$2A:$6C00 histogram: ' + ', '.join('${}={}'.format(v.upper(), n) for v,n in c44.most_common(5)))

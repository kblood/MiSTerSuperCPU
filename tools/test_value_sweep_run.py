"""Run test_value_sweep and dump full UART G:## ## ## + B:## fields.
G byte 1 = bank $00:$6C00 (mem_40)
G byte 2 = bank $2A:$6C00 (mem_44)
G byte 3 = bank $00:$6C03 (mem_5C)
B byte   = bank $2A:$6C03 (mem_45)
Test writes $3E to $2A:$6C00 and $A5 to $2A:$6C03.
Expect: G byte 2 = $3E, B = $A5
"""
import os, time, paramiko, base64, re
from collections import Counter
HOST,USER,PASS = '192.168.50.130','root','1'
RBF='/media/fat/_Test/C64.rbf'; CFG='/media/fat/config/C64.cfg'
PRG_LOCAL=os.path.join(os.path.dirname(os.path.abspath(__file__)),'test_value_sweep.prg')
PRG_REMOTE='/tmp/test_value_sweep.prg'
PRG_MGL='/tmp/load_test_value_sweep.mgl'
OUT=os.path.join(os.path.dirname(os.path.abspath(__file__)),'test_value_sweep_run')
def run(c,cmd,t=30):
...
gv=g_re.findall(out); bv=b_re.findall(out)
print('Total G samples:',len(gv),'B samples:',len(bv))
if gv:
    last=gv[-1]
    print('  G byte 1 ($00:$6C00) = $'+last[0])
    print('  G byte 2 ($2A:$6C00) = $'+last[1]+'  (expect $3E)')
    print('  G byte 3 ($00:$6C03) = $'+last[2])
if bv:
    print('  B       ($2A:$6C03) = $'+bv[-1]+'      (expect $A5)')

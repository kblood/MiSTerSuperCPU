#!/usr/bin/env python3
"""Bisect B: turbo->1MHz toggle BEFORE long-store, NO FETCH (write $3E by hand).
Sequence:
  1MHz on
  TURBO ON
  1MHz ON  <-- toggle
  LDA #$3E; STA $0500  (preload byte 0)
  TURBO ON
  Long-store $0500..$05FF -> bank $2A:$6C00
  1MHz ON
  TURBO ON; native; 16-bit M; LDA long
  Border green if = $3E
"""
import os
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_bisect_b.prg')
...
emit(0x8D, 0x7A, 0xD0)               # 1MHz again (toggle)
emit(0xA9, 0x03, 0x8D, 0x20, 0xD0)   # cyan
# Preload $0500 = $3E (don't use REU FETCH)
emit(0xA9, 0x3E, 0x8D, 0x00, 0x05)
# Turbo + long-store
emit(0x8D, 0x7B, 0xD0)

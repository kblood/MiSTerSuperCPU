"""v289 follow-up: Doom flow → load peek_ead0.prg → screenshot.

Disassembly at $2C:$85B6 shows `LDA $87:$EAD8 / STA $88; LDA $87:$EADA /
STA $8A`, suggesting $87:$EAD8/$EADA hold the saved error-context $88/$8A
(persistent across core reload). After Doom halts via "Bad music number -9",
SDRAM bank $87 retains those bytes — load peek_ead0.prg via MGL to dump
$87:$EAD0..$EAEF on screen.

If $87:$EAD8 holds the saved $88 (music_num low) and $87:$EADA holds saved
$8A (music_num high), expect ($F7, $FF) for music_num = -9 = $FFF7.
"""
...
PEEK_LOCAL = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'peek_ead0.prg')
LOADER_REMOTE = '/tmp/loader.prg'
PEEK_REMOTE = '/tmp/peek_ead0.prg'
REU_REMOTE = '/media/fat/games/C64/doom.reu'
REU_MGL = '/tmp/load_doom_reu.mgl'
LOADER_MGL = '/tmp/load_doom_loader.mgl'
PEEK_MGL = '/tmp/load_peek_ead0.mgl'
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'v289_peek_ead0')
...
def main():
    ...
    print('wait 180s for Doom to halt at "Bad music number -9" ...')
    time.sleep(180)
    print('load peek_ead0.prg (SDRAM bank $87 persists across core reload) ...')

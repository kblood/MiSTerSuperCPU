#!/usr/bin/env python3
"""Bisect C: SAME as test_long_store_only (which PASSED) BUT with 1MHz toggle inserted.
test_long_store_only sequence: 1MHz, turbo, single long-store, ...
bisect_c:                       1MHz, turbo, 1MHz, turbo, single long-store, ...

If this fails, the bug is the 1MHz->turbo->1MHz->turbo sequence breaking long-store.
"""
import os
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'test_bisect_c.prg')
...
emit(0x8D, 0x7A, 0xD0)               # 1MHz (NEW toggle)
emit(0x8D, 0x7B, 0xD0)               # turbo (NEW)
emit(0xA9, 0x03, 0x8D, 0x20, 0xD0)
# Single long-store: A=$3E to bank $2A:$6C00
emit(0xA9, 0x00, 0x85, 0xFB)
emit(0xA9, 0x6C, 0x85, 0xFC)
emit(0xA9, 0x2A, 0x85, 0xFD)
emit(0xA9, 0x3E)
emit(0xA0, 0x00)
emit(0x97, 0xFB)                     # single STA [$FB],Y
# Native + 16-bit, LDA long
emit(0x18, 0xFB)
emit(0xC2, 0x20)

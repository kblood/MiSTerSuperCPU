#!/usr/bin/env python3
"""Run test_fetch_1mhz.prg via separate REU+PRG MGL load.

PASS = green border + UART G byte 2 = $3E (long-store works in 1MHz too).
FAIL = red border + UART G byte 2 = something else (1MHz REU FETCH broken).
"""
...
PRG_LOCAL = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         'test_fetch_1mhz.prg')
PRG_REMOTE = '/tmp/test_fetch_1mhz.prg'
REU_MGL = '/tmp/load_doom_reu_only.mgl'
PRG_MGL = '/tmp/load_test_fetch_1mhz.mgl'
...
def main():
    ...
    if g_vals:
        ...
        print('$2A:$6C00 hist: ' + ', '.join('${}={}'.format(v.upper(), n) for v,n in c44.most_common(5)))

...
# Path B GHDL bench: real cpu_65c816 inside scpu_async_bridge.
# Goal: reproduce (or not) the silicon wedge in sim.
[CmdletBinding()]
param(
...
$workDir   = Join-Path $scriptDir 'work_cpu'
if (-not (Test-Path $workDir)) { New-Item -ItemType Directory -Path $workDir | Out-Null }

# Order matters: package first, then leaf entities, then top, then wrapper, then bridge, then bench.
$sources = @(
    ...
    (Join-Path $scriptDir 'cpu_in_bridge_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags cpu_in_bridge_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $genericArgs = @("-gRATIO=$Ratio", "-gSTOP_TIME_NS=$StopTimeNs")
    $logPath = Join-Path $workDir "cpu_in_bridge_tb_R${Ratio}.log"
    Write-Host ""
    Write-Host "Run: RATIO=$Ratio STOP_TIME_NS=$StopTimeNs"
    & $ghdl -r @ghdlFlags cpu_in_bridge_tb @genericArgs 2>&1 | Tee-Object -FilePath $logPath
    Write-Host ""

#!/usr/bin/env bash
# Cache hit-rate observer: build and run the REAL-fpga64_sid_iec GHDL harness.
#
...
# Deliverables under work_cache/:
#   c64_cache_hitrate_tb.log  simulation transcript
#   c64_cache_hitrate_tb.fst  waveform
...
WORK_DIR="${SCRIPT_DIR}/work_cache"
...
SOURCES=(
    ...
    "${SCRIPT_DIR}/c64_cache_hitrate_tb.vhd"
)
...
echo "==> Elaborate"
"${GHDL}" -e "${GHDL_FLAGS[@]}" c64_cache_hitrate_tb

echo "==> Run (stop-time=${STOP_TIME})"
LOG="${WORK_DIR}/c64_cache_hitrate_tb.log"
WAVE="${WORK_DIR}/c64_cache_hitrate_tb.fst"
set +e
"${GHDL}" -r "${GHDL_FLAGS[@]}" c64_cache_hitrate_tb \
    --fst="${WAVE}" \
    --stop-time="${STOP_TIME}" \

...
# Single-scenario runner for p65c816_sr_emu_wrap_tb.vhd
# Validates emu-mode stack-relative ($A3 LDA sr,S) page wrap fix:
#   VICE: loc = (reg_sp + p1) & 0xffff; if emul then loc = (loc & 0xff) | 0x100
#   Our bug:  DH = SPH + SPL+op carry  -> DX = $02xx when SPL+op overflows page
#   Fix:     DH unchanged in emu mode when ABSCtrl=11 (stack-relative)
[CmdletBinding()]
param(
    [string]$StopTime = "2ms"
)
...
$workDir   = Join-Path $scriptDir 'work_sr'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_sr_emu_wrap_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_sr_emu_wrap_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'sr_emu_wrap.log'
    & $ghdl -r @ghdlFlags p65c816_sr_emu_wrap_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

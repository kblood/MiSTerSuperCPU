...
# Single-scenario runner for p65c816_scpumips_copy_tb.vhd
# Reproduces Doom's SCPUMIPS dispatcher first cross-bank copy:
# `LDA $80:$1234,X` ($BF) / `STA $00:$2000,X` ($9F) loop, 16 bytes.
[CmdletBinding()]
param(
    [string]$StopTime = "2ms"
)
...
$workDir   = Join-Path $scriptDir 'work_scpumips'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_scpumips_copy_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_scpumips_copy_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'scpumips_copy.log'
    & $ghdl -r @ghdlFlags p65c816_scpumips_copy_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

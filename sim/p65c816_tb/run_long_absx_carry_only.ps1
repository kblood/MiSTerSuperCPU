...
# Tests BF/9F long-abs,X with X=$FF -> $00 (low-byte carry into high byte).
# Mirrors Doom prologue $20:$02F2.
[CmdletBinding()]
param([string]$StopTime = "120us")
...
$workDir   = Join-Path $scriptDir 'work_absxcarry'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_long_absx_carry_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_long_absx_carry_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'long_absx_carry.log'
    & $ghdl -r @ghdlFlags p65c816_long_absx_carry_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    Write-Host ""

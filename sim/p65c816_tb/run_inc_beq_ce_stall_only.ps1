...
# CE-stall variant runner for p65c816_inc_beq_ce_stall_tb.vhd
# Tests whether a CE stall (mirroring an unprotected dma_active drop)
# landing inside INC $07B9's dummy-then-real RMW write pair corrupts
# the real write value or the flags BEQ reads.
[CmdletBinding()]
param(
...
$workDir   = Join-Path $scriptDir 'work_cestall'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_inc_beq_ce_stall_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_inc_beq_ce_stall_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'inc_beq_ce_stall.log'
    & $ghdl -r @ghdlFlags p65c816_inc_beq_ce_stall_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

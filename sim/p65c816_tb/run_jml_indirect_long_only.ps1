...
# Single-scenario runner for p65c816_jml_indirect_long_tb.vhd
# Tests opcode $DC = JML [zp]. Doom's SCPUMIPS handler returns via this.
[CmdletBinding()]
param(
    [string]$StopTime = "60us"
)
...
$workDir   = Join-Path $scriptDir 'work_jmlind'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_jml_indirect_long_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_jml_indirect_long_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'jml_indirect_long.log'
    & $ghdl -r @ghdlFlags p65c816_jml_indirect_long_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

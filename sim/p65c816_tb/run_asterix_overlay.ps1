...
$workDir   = Join-Path $scriptDir 'work_asterix_overlay'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_asterix_overlay_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_asterix_overlay_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'asterix_overlay.log'
    & $ghdl -r @ghdlFlags p65c816_asterix_overlay_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

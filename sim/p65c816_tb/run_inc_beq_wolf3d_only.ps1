...
# Single-scenario runner for p65c816_inc_beq_wolf3d_tb.vhd
# Tests whether INC $07B9 / BEQ can mis-evaluate Z with no bus contention.
[CmdletBinding()]
param(
...
$workDir   = Join-Path $scriptDir 'work_incbeq'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_inc_beq_wolf3d_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_inc_beq_wolf3d_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'inc_beq_wolf3d.log'
    & $ghdl -r @ghdlFlags p65c816_inc_beq_wolf3d_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

...
# Tests opcode $5C = JML al cross-bank (Doom's $20:$03E6 -> $80:$005C entry).
[CmdletBinding()]
param([string]$StopTime = "60us")
...
$workDir   = Join-Path $scriptDir 'work_jmllong'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_jml_long_crossbank_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_jml_long_crossbank_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'jml_long_crossbank.log'
    & $ghdl -r @ghdlFlags p65c816_jml_long_crossbank_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    Write-Host ""

...
$workDir   = Join-Path $scriptDir 'work_asterix_full'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_asterix_full_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_asterix_full_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'asterix_full.log'
    & $ghdl -r @ghdlFlags p65c816_asterix_full_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

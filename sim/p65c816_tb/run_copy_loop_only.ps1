...
# Single-scenario runner for p65c816_copy_loop_tb.vhd
# Reproduces the "indexed + INY + BNE" copy-loop hang observed on hardware
# (KERNAL RAMTAS $FD6F-$FD85 and Asterix decompressor $0853-$085A).
[CmdletBinding()]
param(
    [string]$StopTime = "2ms"
)
...
$workDir   = Join-Path $scriptDir 'work_cl'
...
try {
    ...
    $logPath = Join-Path $workDir 'copy_loop.log'
    & $ghdl -r @ghdlFlags p65c816_copy_loop_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

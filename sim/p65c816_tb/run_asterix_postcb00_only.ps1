...
$workDir   = Join-Path $scriptDir 'work_asterix_postcb00'
if (-not (Test-Path $workDir)) { New-Item -ItemType Directory -Path $workDir | Out-Null }

# Ensure asterix.mem exists in workDir (use same generator as full bench)
$memFile = Join-Path $workDir 'asterix.mem'
$srcMem  = Join-Path $scriptDir 'work_asterix_full\asterix.mem'
if (-not (Test-Path $memFile) -and (Test-Path $srcMem)) {
    Copy-Item $srcMem $memFile
}
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_asterix_postcb00_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_asterix_postcb00_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'asterix_postcb00.log'
    & $ghdl -r @ghdlFlags p65c816_asterix_postcb00_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

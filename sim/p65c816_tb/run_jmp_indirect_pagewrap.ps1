...
# Single-scenario runner for p65c816_jmp_indirect_pagewrap_tb.vhd
# Verifies NMOS JMP ($xxFF) page-wrap fix in emu mode.
[CmdletBinding()]
param(
    [string]$StopTime = "2ms"
)
...
$workDir   = Join-Path $scriptDir 'work_jmpind'
...
$sources = @(
    ...
    (Join-Path $scriptDir 'p65c816_jmp_indirect_pagewrap_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags p65c816_jmp_indirect_pagewrap_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'jmp_indirect_pagewrap.log'
    & $ghdl -r @ghdlFlags p65c816_jmp_indirect_pagewrap_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

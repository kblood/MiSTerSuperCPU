...
# Cache latency-skew bench: reproduces the iter-7b cross-line same-cycle stale read.
[CmdletBinding()]
...
$workDir   = Join-Path $scriptDir 'work_latency'
if (-not (Test-Path $workDir)) { New-Item -ItemType Directory -Path $workDir | Out-Null }

$sources = @(
    (Join-Path $rtl 'cpu_cache.vhd'),
    (Join-Path $scriptDir 'cpu_cache_latency_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags cpu_cache_latency_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'cache_latency.log'
    & $ghdl -r @ghdlFlags cpu_cache_latency_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

...
# Bug 2 emergent skew + fix validation (pipelined SDRAM model).
[CmdletBinding()]
...
$workDir   = Join-Path $scriptDir 'work_pipe'
if (-not (Test-Path $workDir)) { New-Item -ItemType Directory -Path $workDir | Out-Null }

$sources = @(
    (Join-Path $rtl 'cpu_cache.vhd'),
    (Join-Path $scriptDir 'cpu_cache_superram_pipe_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags cpu_cache_superram_pipe_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'superram_pipe.log'
    & $ghdl -r @ghdlFlags cpu_cache_superram_pipe_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

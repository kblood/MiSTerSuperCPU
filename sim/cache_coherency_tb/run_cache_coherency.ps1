...
# Cache coherency bench: reproduces v88 stale-read symptom.
[CmdletBinding()]
...
$sources = @(
    ...
    (Join-Path $scriptDir 'cpu_cache_coherency_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags cpu_cache_coherency_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'cache_coherency.log'
    & $ghdl -r @ghdlFlags cpu_cache_coherency_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

...
# Bug 2 hunt: bank-$20 read staleness — coherence replay + fill-skew probe.
[CmdletBinding()]
...
$workDir   = Join-Path $scriptDir 'work_replay'
if (-not (Test-Path $workDir)) { New-Item -ItemType Directory -Path $workDir | Out-Null }

$sources = @(
    (Join-Path $rtl 'cpu_cache.vhd'),
    (Join-Path $scriptDir 'cpu_cache_bank20_replay_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags cpu_cache_bank20_replay_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'bank20_replay.log'
    & $ghdl -r @ghdlFlags cpu_cache_bank20_replay_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

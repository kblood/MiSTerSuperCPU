...
# Bug 2 iter-18 fill-data-phase reproduction + delayed-fill fix proof.
[CmdletBinding()]
param([string]$StopTime = "5ms")
...
$workDir   = Join-Path $scriptDir 'work_filldata'
if (-not (Test-Path $workDir)) { New-Item -ItemType Directory -Path $workDir | Out-Null }

$sources = @(
    (Join-Path $rtl 'cpu_cache.vhd'),
    (Join-Path $scriptDir 'cpu_cache_filldata_phase_tb.vhd')
)
...
try {
    ...
    & $ghdl -e @ghdlFlags cpu_cache_filldata_phase_tb
    if ($LASTEXITCODE -ne 0) { throw "elab failed" }

    $logPath = Join-Path $workDir 'filldata_phase.log'
    & $ghdl -r @ghdlFlags cpu_cache_filldata_phase_tb --stop-time=$StopTime 2>&1 | Tee-Object -FilePath $logPath
    $rc = $LASTEXITCODE
    Write-Host ""

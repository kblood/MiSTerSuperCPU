...
- **Side-quest result (evening 2026-05-27): Doom autoload solved via MGL + single PRG (v356).** `tools/_doom_autoload.mgl` = REU + `doom_loader.prg`, MiSTer's start_strk fires R-U-N + RETURN, BASIC SYS-dispatches into doom_loader's inner ML, REU FETCH chain transfers Doom into SuperRAM, XCE + JML $20:0000 launches. Silicon-validated reaching the playable main menu unattended at ~t220 s. Same pattern as Wolfenstein. The CRT-based v1/v2/v3 attempts (in commits `f6ee52e`, `7866e3d`) all dead-end because cart-direct-JMP skips BASIC SYS context the loader's terminal JML depends on. HEAD blocker: `start_strk` was wiped by commit `58f9dc3` (vanilla restore) — restoring deferred-start_strk from `8713403` and latched ioctl classification from `0bcfabe` in c64.sv is the right unblock. Probe: `tools/doom_autoload_probe.py`.

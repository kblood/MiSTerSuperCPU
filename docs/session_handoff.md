...
- **Side-quest result (evening 2026-05-27): Doom autoload v3 silicon-validated on v356.** `tools/test_cart/gen_doom_autoload_crt_v3.py` builds a BASIC-free CRT — cart bootstrap copies doom_loader's inner ML (187 bytes from $0820-$08DA) directly to $0700 and JMPs there. No BASIC interpreter, no SYS dispatch, no keystroke synthesis. Validated on v356 RBF showing full Doom title screen at t200s, fully unattended. Commit `f6ee52e`. Replaces the brittle MGL+PRG+start_strk+mhold path. Will work on HEAD the moment the SDRAM A10 bug is fixed — autoloader needs no further changes.
